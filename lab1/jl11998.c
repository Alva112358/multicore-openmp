...
double sequential_version(const double* nums, const size_t random_num, const size_t nums_bin) {
    ...
    for (size_t i = 0; i < random_num; ++i) {
        size_t bin = (size_t)(nums[i] / (double)nums_bin);
        histogram[bin] += 1;
    }
...
double thread_bin_version(const double* nums, const size_t random_num, const size_t nums_bin) {
    ...
    double start = omp_get_wtime();
    #pragma omp parallel for default(none) shared(bin_size, random_num, nums, nums_bin, histogram)
...
double thread_subset_version(const double* nums, const size_t random_num, const size_t nums_bin, const size_t num_thread) {
    size_t bin_size = (size_t)(100.0 / (double) nums_bin);
    size_t* histogram = (size_t*)malloc(bin_size * sizeof(size_t));
    memset(histogram, 0, bin_size * sizeof(size_t));

    size_t** histogram_private = (size_t**)malloc(num_thread *sizeof(size_t*));
    for (size_t i = 0; i < num_thread; ++i) {
        histogram_private[i] = (size_t*)malloc(bin_size * sizeof(size_t));
        memset(histogram_private[i], 0, bin_size * sizeof(size_t));
    }

    double start = omp_get_wtime();
#pragma omp parallel default(none) shared(bin_size, random_num, nums, histogram, nums_bin, num_thread, histogram_private)
    {
        size_t tid = omp_get_thread_num();
#pragma omp for
        for (size_t i = 0; i < random_num; ++i) {
            size_t bin = (size_t)(nums[i]/(double)nums_bin);
            histogram_private[tid][bin] += 1;
        }
#pragma omp for
        for (size_t i = 0; i < bin_size; ++i) {
            for (size_t t = 0; t < num_thread; ++t) {
                histogram[i] += histogram_private[t][i];
            }
        }
    };
    double end = omp_get_wtime();
...
int main(int argc, char* argv[]) {
    ...
    double sequential_time = sequential_version(nums, random_num, nums_bin);
    printf("Time: %f\n\n", sequential_time);
    ...
    /* 3. Each thread is responsible for a subset of number. */
    printf("3.Subset Version:\n");
    double subset_time = thread_subset_version(nums, random_num, nums_bin, nums_thread);
    printf("Time: %f\n\n", subset_time);
